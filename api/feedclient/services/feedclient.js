...
async function selectNewProposalsNews(creator, maxUserFeed) {
    let userFeeds;
    if (maxUserFeed) {
...
async function selectLikeProposalsNews(target, maxFollow) {
    let follows;
    if (maxFollow) {
...
async function selectMyProposalsNews(target) {
...
async function selectMyCommentsNews(target) {
    const follows = await strapi.query('follow').find({
        target,
        isFeedActive: true,
        type: 'POST',
...
async function processComment(target, activity, memberName, type) {
...
module.exports = {
    ...
    async onPostCreated(post) {
        ...
        if (type === 'NEW_OPINION_COMMENT') {
            await processComment(post.parentPost.id, post.activity.id, post.writer.username, type);
    ...
    async onInteractionCreated(interaction) {
        ...
        await processComment(interaction.post.id, interaction.post.activity, interaction.actor.username, type);
    },
};
