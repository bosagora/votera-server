...
async function waitForProposalMiningTransaction(state, proposal, method) {
    ...
    await Promise.all(
        txs.map((tx) => {
            return strapi.services.boaclient.waitForTransactionReceipt(tx.transactionHash).then((receipt) => {
...
async function changeProposalStatusFromTo(proposal, from, to) {
    ...
    if (result && NOTIFY_CHANGED_STATUS.includes(to)) {
        try {
            const subProposal = {
                id: result.id,
                name: result.name,
            ...
            strapi.services.pubsub.publish('proposalChanged', subProposal).catch((err) => {
                strapi.log.warn(`publish.proposalChanged failed: proposal.id = ${result.id}`);
                strapi.log.warn(err);
            });
...
module.exports = {
    ...
    async checkProposalFee(proposal, transactionHash) {
        try {
            const methodName =
                proposal.type === ENUM_PROPOSAL_TYPE_BUSINESS ? 'createFundProposal' : 'createSystemProposal';
